...
def set_theme(current_theme):
    ...
    if current_theme == get_translation("WINTER"):
        theme = pygame.image.load("winter_landscape.png")
    elif current_theme == get_translation("SUMMER"):
        theme = pygame.image.load("summer.jpg")
    elif current_theme == get_translation("AUTUMN"):
        theme = pygame.image.load("fall.jpg")
    elif current_theme == get_translation("SPRING"):
        theme = pygame.image.load("spring.jpg") 
    ...
    def level_selection():
        ...
        while running:
            ...
            def save_data(name, level):
                data = {'name': name, 'level': level, 'score': 0}
                try:
...
def Pacman(level_color, level_speed):
    ...
    def draw_misc():
        ...
        screen.blit(score_text, (10, 930)) # screen.blit = putting it on the screen, (10,920) = position of "score"
        if powerup: # if pacman eats a big dot
            pygame.draw.circle(screen, 'blue', (140, 930), 15) 
        for i in range(lives):
            screen.blit(pygame.transform.scale(player_images[0], (30, 30)), (950 + i * 40, 930))
        if game_over:
            subprocess.Popen([sys.executable, "gameover.py"])
        if game_won:
